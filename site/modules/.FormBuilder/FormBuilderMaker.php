...
class FormBuilderMaker extends Wire {
	...
	protected function makeForm(array $data, $language) {
		...
		foreach($data as $k => $v) {
			// if the form has properties that match those in the processor, set them to the procesor	
	 ...
	 *
	 */
	protected function makeInputfield($type, $name, array $data, $language) {
		...
		if($language) {
			foreach(array('label', 'description', 'notes', 'placeholder') as $key) {
