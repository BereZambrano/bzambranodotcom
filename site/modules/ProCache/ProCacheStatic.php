 ...
 * @method string getCachePathRoot()
 *
 */
...
class ProCacheStatic extends ProCacheClass {
	...
	public function findCacheEntries(array $options = array()) {
		...
		while($row = $query->fetch(\PDO::FETCH_ASSOC)) {
			...
			$entry = $this->newEntry($row); // ProCacheStaticEntry
	...
	public function allowCacheForPage(Page $page) {
		...
		if($user && $user->isLoggedin()) {
			...
			// if any GET vars are present, then abort
			$cache = false;
			$varNames = $procache->noCacheGetVars;
			if(strlen($varNames) && trim($varNames) !== '*') {
				// @todo determine whether we should allow for this option
				/*
				$cache = true;
				foreach(explode("\n", $varNames) as $name) {
					$name = trim($name);
			...
			// if POST vars are present, then abort
			$cache = false;
	 ...
	 * 
	 */
	public function ___getCachePathRoot() {
	...
	public function getCachePath(Page $page = null, array $options = array()) {
		...
		if(!empty($options['urlSegments'])) {
			$sanitizer = $this->wire()->sanitizer;
			foreach($options['urlSegments'] as $segment) {
				$segments[] = $sanitizer->pagePathNameUTF8($segment);
			}
		}
	...
	public function columnExists($column, $table = '') {
		$database = $this->wire()->database;
		$table = $table ? $database->escapeTable($table) : ProCache::DB_TABLE;
