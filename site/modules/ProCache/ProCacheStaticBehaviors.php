...
class ProCacheStaticBehaviors extends ProCacheClass {
	...
	protected $behaviorAbbrs = array(
		...
		ProCache::CACHE_CLEAR_REFERENCING => 'N',
	...
	protected $behaviorNames = array(
		...
		ProCache::CACHE_CLEAR_REFERENCING => 'referencing',
	...
	*/
	...
	public function getCacheClearBehaviors(array $options = array()) {
		...
		foreach($cacheTemplates as $t) {
			...
			if(!$t instanceof Template) continue;
			...
			if(strlen($behaviorStr)) {
				...
				if(strpos($behaviorStr, ',') !== false) {
					...
					foreach(explode(',', $behaviorStr) as $c) {
						$c = trim($c);
						if(strlen($c) > 1 && !ctype_digit("$c")) $c = $c[0];
						$behaviors[] = $c;
					}
				} else {
					// format: "CPH" or "123" or "11", etc.
					for($n = 0; $n < strlen($behaviorStr); $n++) {
						$v = $behaviorStr[$n];
						if(isset($behaviorStr[$n+1]) && $behaviorStr[$n+1] === $v) {
							// same digits count as one value, i.e. "11", "22", etc.
							$behaviors[] = "$v$v";
							$n++;
						} else {
							$behaviors[] = $v;
	...
	public function ___executeCacheClearBehaviors(Page $page, array $behaviors) {
		...
		if(!empty($behaviors[ProCache::CACHE_CLEAR_REFERENCING])) {
			// other pages that $page is referencing
			$items = $page->referencing;
			if($items->count()) {
				$cleared['referencing'] = $static->clearPages($items);
			} else {
				$cleared['referencing'] = 0;
			}
		}
