...
class CSS extends Minify
{
    ...
    protected function combineImports($source, $content)
    {
        ...
        foreach ($matches as $match) {
            ...
            if (@file_exists($importPath) && is_file($importPath)) {
                ...
                $minifier = new self($importPath); // RJC
    ...
    protected function execute($path = null)
    {
        ...
        foreach ($this->data as $source => $css) {
	        ...
	        $noMinify = strpos($css, '/*NoMinify*/') === 0; // RJC
	        if(!$noMinify) {
		        $this->extractStrings();
		        $this->stripComments();
    ...
    protected function shortenZeroes($content)
    {
	    ...
	    /*** RJC commented out June 23, 2022 and replaced with the code following this comment
        $before = '(?<=[:(, ])';
        $after = '(?=[ ,);}])';
        // $units = '(em|ex|%|px|cm|mm|in|pt|pc|ch|rem|vh|vw|vmin|vmax|vm)';
	    $units = '(em|ex|px|cm|mm|in|pt|pc|ch|rem|vh|vw|vmin|vmax|vm)'; // RJC removed "%" because it's not a <length> unit
