...
function setupFormBuilderSubmitted($) {
    ...
    } else if (UIkit !== undefined && window.parent.UIkit !== undefined) {
        // scroll just to top if no jQuery to use
        var localUtil = UIkit.util;
        var parentUtil = window.parent.UIkit.util;
        var formName = localUtil.attr(localUtil.$("form.InputfieldForm[id^=FormBuilder_]"), 'name') // @todo
        if (!formName) formName = localUtil.attr(localUtil.$("#FormBuilderSubmitted"), 'data-name');
        //var $viewport = window.parent.jQuery('#FormBuilderViewport_' + formName);
        var $viewport = parentUtil.$('#FormBuilderViewport_' + formName);
        //console.log($viewport.getBoundingClientRect());
        //
        parentUtil.scrollIntoView($viewport, {duration: 1000, offset: y});
