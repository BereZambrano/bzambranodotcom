 ...
 * @method array formToEntry(InputfieldForm $form, $id = 0) Given InputfieldForm, convert to $entry array (as called by saveForm method). 
 ...
 * @method string renderErrors(array $errors = array()) Render error messages. 
...
class FormBuilderProcessor extends WireData {
	...
	const saveFlagGoogleSheets = 128;
	...
	public function ___render($id = null) {
		...
		if(!$this->forms->forms()->isValidLicense()) {
			$this->initErrors[] = $this->_('No valid product key detected');
		}

		if(!$fbForm->hasPermission('form-submit')) {
			$this->initErrors[] = $this->_('This form is not available at your access level.');
		}
		
		if($isDisabled && !$isPreview) {
			$this->initErrors[] = $this->_('This form is disabled.');
		}
		...
		if($submitType === FormBuilderMaker::submitTypeNone) {
			...
			if($this->processInput($entryID, $submitType)) {
				// processInput returns true on successful final processing of form
				if($isEmbedD) {
				...
				// form was submitted but either there were errors or it's a pagination submit
			}
		...
		$form->appendMarkup .= $this->renderSubmitKey() . $this->appendMarkup;

		if($isExportD) {
			// generate the embed method D file
			$out .= $this->renderExportEmbedMethodD($form, $framework);
			...
			$out .= $this->renderPreview($form);

		} else if($isEmbedD) {
			// we are rendering from a custom markup file in /site/templates/FormBuilder/	
			$out .= $this->renderEmbedMethodD($form, $fileEmbedD, $framework, $submitType, $errors);
	 ...
	 * 
	 */
	protected function renderEmbedMethodD(InputfieldForm $form, $formFile, $framework, $submitted, array $errors) {
		...
		return $this->renderReady($form, $formFile, array(
			'submitted' => $submitted,
			'errors' => $errors,
			'values' => $values,
			'labels' => $labels,
	...
	/**
	 * Return true if submit key in POST is valid, false if not
	 * 
	 * @param bool $testOnly
	 * @param bool $verbose
	 * @return bool
	 * 
	 */
	public function validSubmitKey($testOnly = false, $verbose = true) {

		// return cached value if available
		if($this->validSubmitKey !== null) return $this->validSubmitKey;

		// first check if form posted
		$a = $this->maker->parseSubmitKey($this->wire('input')->post('_submitKey'));

		if($a['qty'] < 3) {
			if($verbose) $this->adminError("Submit key not present or too small");
			return false;
		}

		// if formName doesn't match up, it's not valid
		if($a['formName'] !== $this->formName) {
			if($verbose) $this->adminError("Wrong form name (expected '$this->formName', received '$a[formName]') in _submitKey");
			return false;
		}
		...
		if($this->validateNumFields) {
			// if number of fields doesn't match up, it's not valid
			$numChildren = count($this->form->children);
			if($a['numFields'] != $numChildren) {
				if($verbose) $this->adminError("Number of fields in _submitKey ($a[numFields]) does not match number ($numChildren) in form");
				return false;
			}
		}
		...
		if($this->skipSessionKey) {
			...
			if($a['sessionKey'] != "0") {
				if($verbose) $this->adminError("Skip value '0' expected but not present in _submitKey");
				return false;
			} else {
			...
			if($a['sessionKey'] === $sessionKeyLast) {
				// session key matched
				...
				if($a['sessionKey'] === $this->sessionGet('sessionKeyLast')) {
					// if so, we'll acknowledge it
					$this->addError($this->_('This form was already submitted.'));
					if($verbose) $this->adminError("Session key in _submitKey duplicates previously used one");
				} else if($verbose) {
					$this->adminError("Session key in _submitKey does not match one submitted with form");
				}
				return false;
			}
		}
	...
	protected function processInputTuringTest() {
		...
		foreach($this->turingTest as $fieldName => $answer) {
			...
			if(strtolower($field->attr('value')) != strtolower($answer)) $field->error($this->_('Incorrect answer')); 
		}
	}
	...
	protected function ___saveForm(InputfieldForm $form, $id = 0) {
		...
		if($this->saveFlags & self::saveFlagEmail) {
			if(!$this->emailForm($form, $data)) {
				$this->addError($this->_('Unable to verify successful email delivery of this form submission.'));
			}
		}			
	 ...
	 *
	 */
	public function ___formToEntry(InputfieldForm $form, $id = 0) {
		$data = array();
	 ...
	 *
	 */
	public function ___renderErrors($errors = array()) {
		...
		if(empty($errors)) {
			$errors = $this->getErrors();
			if(empty($errors)) $errors = $this->getErrors(true); // get field specific errors
	 ...
	 * 
	 */
	protected function ___renderSubmittedLandmark() {
	...
	public function addError($text, $prepend = false) {
		if(in_array($text, $this->errors)) return;
		if($prepend) {
